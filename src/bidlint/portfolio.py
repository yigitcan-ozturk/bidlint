...
def portfolio_to_markdown(reports: list[ComplianceReport]) -> str:
    """Render a review-friendly multi-vendor Markdown tabulation."""
    ranked = rank_reports(reports)
    if not ranked:
        raise ValueError("portfolio report requires at least one vendor report")

    lines = [
        "# bidlint technical bid tabulation",
        "",
        f"- **Specification:** {_markdown_cell(ranked[0].specification)}",
        f"- **Vendors:** {len(ranked)}",
        f"- **Generated by:** bidlint v{__version__}",
        "",
        "## Vendor ranking",
        "",
        "| Rank | Vendor | Score | PASS | DEVIATION | MISSING | REVIEW |",
        "| ---: | --- | ---: | ---: | ---: | ---: | ---: |",
    ]
    for index, report in enumerate(ranked, start=1):
        counts = report.counts
        lines.append(
            f"| {index} | {_markdown_cell(report.vendor)} | {report.compliance_score:.1f}% | "
            f"{counts['PASS']} | {counts['DEVIATION']} | {counts['MISSING']} | {counts['REVIEW']} |"
        )
    ...
    vendor_headers = " | ".join(_markdown_cell(report.vendor) for report in ranked)
    lines.extend(
        [
            "",
            "## Requirement-by-vendor matrix",
            "",
            f"| Requirement | {vendor_headers} |",
            f"| --- | {' | '.join('---' for _ in ranked)} |",
        ]
    )

    for requirement_id in requirement_ids:
        label = _markdown_cell(requirement_labels[requirement_id])
        required = _markdown_cell(_required_text(ranked[0], requirement_id))
        cells: list[str] = []
        for finding_map in vendor_maps:
            finding = finding_map.get(requirement_id)
            if finding is None:
                cells.append("**MISSING**<br>—")
                continue
            fact = finding.vendor_fact
            offered = _markdown_cell(fact.raw_value if fact else "—")
            reason = _markdown_cell(finding.reason)
            cells.append(f"**{finding.status.value}**<br>{offered}<br>{reason}")
        lines.append(f"| **{requirement_id}** {label}<br>{required} | {' | '.join(cells)} |")

    lines.extend(
        [
            "",
            "> Technical ranking only. Commercial price, delivery, payment terms and engineering acceptance are outside bidlint.",
            "",
        ]
    )
    return "\n".join(lines)
